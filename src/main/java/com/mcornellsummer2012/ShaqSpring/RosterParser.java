...
public class RosterParser {
	...
	private void writeAdjList () {
		try {
			this.output = new BufferedWriter(new FileWriter("/Users/matthewcornell/Documents/CIS120Workspace/Nets 150 Eclipse Workspace/NetsHW5/files/AdjList.txt", false));
		} catch (IOException e) {
	...
	private TreeMap<String, String> getPlayersByLetter (char letter) {
		...
		for (Element row : rows) {
			...
			//CALLS HELPER FUNCTION TO SORT OUT WIERD CHARACTERS
			if (name.contains("�")) {
